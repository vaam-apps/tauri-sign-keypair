...
use tauri_plugin_sign_keypair::{KeyBacking, KeyProtection, SignKeypairExt, SignerErrorCode};
...
///
/// The software store is a file in the app data directory, so it outlives the
/// test binary; a fixed alias would make the second run fail with
...
fn a_generated_signature_verifies_against_the_reported_jwk() {
    let app = app();
    let key_id = unique_key_id("verify");
    ...
    signer.delete_key(&key_id).expect("cleanup must succeed");
}
...
fn require_hardware_is_refused_not_degraded() {
    let app = app();
    let key_id = unique_key_id("require_hardware");

    let error = app
        .sign_keypair()
        .generate_key(&key_id, true, false, KeyProtection::Ambient)
    ...
    assert!(
        app.sign_keypair().get_key(&key_id).unwrap().is_none(),
...
fn user_presence_is_refused_not_faked() {
    let app = app();
    let key_id = unique_key_id("user_present");

    let error = app
        .sign_keypair()
        .generate_key(&key_id, false, false, KeyProtection::UserPresent)
        .expect_err("a software backend must not pretend to enforce user presence");

    assert_eq!(error.code, SignerErrorCode::HardwareUnavailable);
    assert!(app.sign_keypair().get_key(&key_id).unwrap().is_none());
}

#[test]
fn generate_without_overwrite_refuses_to_clobber_an_existing_key() {
    let app = app();
    let key_id = unique_key_id("clobber");
    let signer = app.sign_keypair();
...
fn a_deleted_key_cannot_sign_and_deleting_twice_is_a_no_op() {
    let app = app();
    let key_id = unique_key_id("deleted");
    let signer = app.sign_keypair();
...
fn keys_survive_a_plugin_restart() {
    let key_id = unique_key_id("persistence");
    ...
    second.sign_keypair().delete_key(&key_id).unwrap();
}
